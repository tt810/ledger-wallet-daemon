...
class WalletPoolsApiTest extends APIFeatureTest {
  ...
  test("WalletPools#Create and list multiple pool") {
    server.httpPost("/pools/your_pool/create", "", headers = defaultHeaders)
    server.httpPost("/pools/this_pool/create", "", headers = defaultHeaders)
    val pools = parse[List[models.Pool]](server.httpGet("/pools", headers = defaultHeaders))
    assert(pools.size == 2)
  ...
  test("WalletPool#Get single pool") {
    server.httpPost("/pools/anotha_pool/create", "", headers = defaultHeaders)
